...
*
* for this homework we will be working with the MCS
...
proc glm data=helpmkh;
  class treat;
  model mcs mcs1 mcs2 mcs3 mcs4 = treat / nouni;
...
proc transpose data=helpmkh_sortid out=helpmkh_mcslong;
  var mcs mcs1 mcs2 mcs3 mcs4;
  by id treat;
  run;

proc contents data=helpmkh_mcslong; run;

proc freq data=helpmkh_mcslong;
  table _name_;
  run;

data help_mcslong;
  set helpmkh_mcslong;
  rename _label_ = mcslabel;
  rename _name_ = mcstime;
  rename col1 = mcs;
  run;

proc contents data=help_mcslong; run;

data help2;
  set help_mcslong;
  if mcstime='mcs'  then time=0;
  if mcstime='mcs1' then time=1;
  if mcstime='mcs2' then time=2;
  if mcstime='mcs3' then time=3;
  if mcstime='mcs4' then time=4;
  run;
...
data reshape(keep=time mcs mean);                                                                                                      
   set meansout;                                                                                                                        
   mcs=mean;  
   output;                                                                                                                              
...
proc gplot data=reshape;                                                                                                                
   plot mcs*time mean*time / overlay haxis=axis1 vaxis=axis2;     
run;  
...
data reshape(keep=treat time mcs mean);                                                                                             
   set meansout;                                                                                                                        
   by treat time;                                                                                                                    
...
/* Define the axis characteristics */                                                                                                   
   ...
   axis2 label=(angle=90) order=(29 to 49 by 5) minor=(n=1);                                                                                                                   
...
/* and overlay symbols at the means. */                                                                                                 
proc gplot data=reshape;                                                                                                                
   plot mcs*time=treat / haxis=axis1 vaxis=axis2 legend=legend1;                                                                    
...
proc mixed data=help2 method=REML covtest;
  class treat;
  model mcs = treat time treat*time / solution;
...
proc mixed data=help2 method=REML covtest plots=all;
  class treat time;
  model mcs = treat time treat*time;
  random intercept /  subject=id type=vc;
