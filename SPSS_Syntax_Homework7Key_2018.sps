* Encoding: UTF-8.
* ======================================.
* N736 - HOMEWORK 7 - Repeated Measures ANOVA
...
* we'll focus on the 5 MCS measurements over time
...
GLM mcs mcs1 mcs2 mcs3 mcs4 BY treat
...
* NOTE - see details on the correction when sphericity
* fails (i.e. when it is significant) - in general
* the Greenhouse-Geisser is often used.
* read more at:
* https://statistics.laerd.com/statistical-guides/sphericity-statistical-guide-2.php

* error-bar plot of mcs at 5 time points
* listwise deletion used as in RM-ANOVA
* the PANEL option is used to see the differences by group.

GRAPH
  /ERRORBAR(CI 95)=mcs mcs1 mcs2 mcs3 mcs4
...
* from a wide format where mcs,mcs1,mcs2,mcs3,mcs4 are all
* in different columns, to instead "stack" these 5 variables
* on top of one another - so each row is a different time point
* and we create one new mcs variable.
* ==========================================.

VARSTOCASES
  /MAKE mcs FROM mcs mcs1 mcs2 mcs3 mcs4
  /INDEX=Time "Time Points"(5) 
  /KEEP=id treat age female pss_fr racegrp homeless a15a a15b d1 
    e2b g1b i1 i2 pcs f1a f1b f1c f1d 
    f1e f1f f1g f1h f1i f1j f1k f1l f1m f1n f1o f1p f1q f1r f1s 
    f1t cesd indtot drugrisk sexrisk 
    satreat substance drinkstatus daysdrink anysubstatus daysanysub 
    linkstatus dayslink e2b1 g1b1 i11 
    pcs1 cesd1 indtot1 drugrisk1 sexrisk1 pcrec1 e2b2 g1b2 i12 
    pcs2 cesd2 indtot2 drugrisk2 sexrisk2 
    pcrec2 e2b3 g1b3 i13 pcs3 cesd3 indtot3 drugrisk3 sexrisk3 
    pcrec3 e2b4 g1b4 i14 pcs4 cesd4 indtot4 
...
SAVE OUTFILE='C:\MyGithub\N736Fall2017_Homework7Key\archive\helpmkh_mcslong.sav'
  /COMPRESSED.
...
* everyone who had MCS data.

FREQUENCIES VARIABLES=mcs
  /NTILES=4
  /STATISTICS=STDDEV MINIMUM MAXIMUM MEAN
  /HISTOGRAM
  /ORDER=ANALYSIS.

* there are 1424 MCS data points and 841 missing out of the 2265 possible 
...
* plot these MCS time profiles for these few subjects.

GRAPH
  /SCATTERPLOT(BIVAR)=time0 WITH mcs BY id
  /MISSING=LISTWISE.
...
MIXED mcs BY treat_flip WITH time0
  /CRITERIA=CIN(95) MXITER(100) MXSTEP(10) SCORING(1) SINGULAR(0.000000000001) HCONVERGE(0, 
    ABSOLUTE) LCONVERGE(0, ABSOLUTE) PCONVERGE(0.000001, ABSOLUTE)
